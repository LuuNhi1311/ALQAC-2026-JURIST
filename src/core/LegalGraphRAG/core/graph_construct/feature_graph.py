...
def summarize_texts(model, text):
    from ALQAC.src.core.LegalGraphRAG.core.prompt import SUMMARIZE_TEXTS_PROMPT
...
def rerank_clusters(model, clusters, query_text):
    from ALQAC.src.core.LegalGraphRAG.core.prompt import RERANK_CLUSTERS_PROMPT_TEMPLATE
    cluster_summaries = "\n".join(
...
def rerank(model, query_text, neighbors):
    from ALQAC.src.core.LegalGraphRAG.core.prompt import RERANK_PROMPT_TEMPLATE
    if not neighbors:
        return []
